...
public class Chomsky {
    ...
    public void eliminateEpsilonProductions() {
        ...
        for (Map.Entry<String, List<String>> entry : productions.entrySet()) {
            ...
            for (String production : oldProductions) {
                if (!production.equals("ε")) {
                    for (String nullable : nullables) {
                        if (production.contains(nullable)) {
                            String newProduction = production.replace(nullable, "");
            ...
            newProductionsList.removeIf(production -> production.equals(""));
    ...
    public void toCnf() {
        ...
        for (String var : productions.keySet()) {
            newProductions.put(var, new ArrayList<>());
            ...
            for (String prod : productions.get(var)) {
                if (prod.length() >= 3) {
                    ...
                    for (int i = 0; i < prod.length() - 2; i++) {
                        ...
                        newProductions.putIfAbsent(newVar, new ArrayList<>());
                        newProductions.get(newVar).add(prod.charAt(i + 1) + prodVars.get(i + 1));
                    }
                    newProductions.putIfAbsent(prodVars.get(prodVars.size() - 1), new ArrayList<>());
